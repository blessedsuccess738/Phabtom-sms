      ...
      <aside className="w-64 border-r border-border bg-card flex flex-col z-10 shadow-2xl">
        ...
        <div className="p-4">
          ...
          <nav className="space-y-1">
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} className={`flex items-center px-3 py-2 rounded-md transition-colors ${active ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-accent/5'}`}>
        ...
        <div className="mt-auto p-4 border-t border-border">
          <div className="text-xs text-muted-foreground font-semibold mb-3 uppercase tracking-wider flex justify-between items-center">
            ...
            <span className={`w-2 h-2 rounded-full ${health?.status === 'ok' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)]' : 'bg-destructive shadow-[0_0_8px_rgba(239,68,68,0.8)]'}`} title={`API: ${health?.status || 'Unknown'}`} />
          </div>
          {status ? (
